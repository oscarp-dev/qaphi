  ...
  return (
    ...
    >
      <Reveal className="flex-1 basis-[280px] max-w-[380px] flex flex-col gap-8">
        ...
        <dl className="flex flex-col gap-5 pt-8 border-t border-ink/15">
          {SPECS.map((spec) => (
            <div key={spec.label} className="flex flex-col gap-1.5">
              <dt className="text-[11px] font-medium text-ink/70 tracking-[.12em] uppercase">{spec.label}</dt>
      ...
      <Reveal delay={150} className="relative flex-none w-[min(58vw,600px)] max-w-full">
        <div className="relative aspect-square overflow-hidden border border-ink/15">
          <img
            src={cafeBolsa}
        ...
        <div className="absolute -right-[10%] -bottom-[14%] w-[52%] aspect-[4/5] overflow-hidden border-[6px] border-cream shadow-[0_18px_40px_rgba(0,0,0,0.25)]">
          <img
            src={cafeDetalle}
